...
**注意**：需要确认 `PermissionHelper` 中是否已有 `CheckControlDevice()` 方法，如果没有需要添加。
...
3. **权限检查**（如需要）
   ...
   - 添加：`bool CheckControlDevice();`（如果不存在）
...
### 阶段 3：服务端集成（0.5 天）
1. IDL 声明添加
2. 服务端权限检查实现（确认 CheckControlDevice 方法）
3. IPC 消息处理
...
**前置条件**：
1. 确认 `PermissionHelper::CheckControlDevice()` 方法是否存在
