...
{
    CALL_DEBUG_ENTER;
    if (!PER_HELPER->VerifySystemApp(data)) {
...
int32_t MultimodalInputConnectStub::StubInjectKeyEvent(MessageParcel& data, MessageParcel& reply)
{
    ...
    if (!isNativeInject && !PER_HELPER->VerifySystemApp(data)) {
        MMI_HILOGE("Verify system APP failed");
        return ERROR_NOT_SYSAPI;
    }
...
int32_t MultimodalInputConnectStub::VerifyTouchPadSetting(MessageParcel& data)
{
    if (!IsRunning()) {
        MMI_HILOGE("Service is not running");
        return MMISERVICE_NOT_RUNNING;
