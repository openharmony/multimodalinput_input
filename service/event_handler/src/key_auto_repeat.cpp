...
void KeyAutoRepeat::SelectAutoRepeat(const std::shared_ptr<KeyEvent>& keyEvent)
{
    ...
    if (keyEvent_ != nullptr) {
        isSameKeyDown = ((keyEvent->GetKeyAction() == KeyEvent::KEY_ACTION_DOWN) && (TimerMgr->IsExist(timerId_)) &&
            (keyEvent->GetKeyCode() == repeatKeyCode_));
    }
    keyEvent_ = keyEvent;
    ...
    if (JudgeKeyEvent(keyEvent_) && TimerMgr->IsExist(timerId_)) {
        ...
        if (!JudgeLimitPrint(keyEvent_)) {
            ...
            MMI_HILOGI("Stop autorepeat, keyCode:%{private}d, repeatKeyCode:%{private}d, keyAction:%d",
                keyEvent_->GetKeyCode(), repeatKeyCode_, keyEvent_->GetKeyAction());
        }
