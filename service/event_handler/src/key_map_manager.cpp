...
void KeyMapManager::GetConfigKeyValue(const std::string &fileName, int32_t deviceId)
{
    ...
    MMI_HILOGD("Number of loaded config files:%{public}uz", configKeyValue_.size());
}
...
void KeyMapManager::RemoveKeyValue(struct libinput_device *device)
{
    ...
    MMI_HILOGD("Number of files that remain after deletion:%{public}uz", configKeyValue_.size());
}
