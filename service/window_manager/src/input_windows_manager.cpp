    ...
    if ((firstBtnDownWindowId_ == -1) ||
        ...
        for (const auto& item : logicalDisplayInfo->windowsInfo) {
            if ((item.flags & WindowInfo::FLAG_BIT_UNTOUCHABLE) == WindowInfo::FLAG_BIT_UNTOUCHABLE) {
                MMI_HILOGD("Skip the untouchable window, window:%{public}d, flags:%{public}d", item.id, item.flags);
                continue;
            } else if ((targetWindowId < 0) && (IsInsideWindow(globalX, globalY, item))) {
                firstBtnDownWindowId_ = item.id;
                MMI_HILOGW("Select the dispatch window of this pointerevent when the targetWindowId "
                ...
                MMI_HILOGW("Select the dispatch window of this pointerevent when the mouse is dragged, or "
                           "can't find the one, window:%{public}d", firstBtnDownWindowId_);
            }
        }
    }
