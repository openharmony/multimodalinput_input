...
constexpr int32_t FIRST_TOUCH { 0 };
...
{
    MMI_HILOGD("Bind cfg file name:%{private}s", BIND_CFG_FILE_NAME.c_str());
#if defined(OHOS_BUILD_ENABLE_POINTER) || defined(OHOS_BUILD_ENABLE_TOUCH)
...
void InputWindowsManager::ReissueCancelTouchEvent(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    for (const auto &item : items) {
        ...
        auto devIter = infos.find(item.GetDeviceId());
        if (devIter == infos.end()) {
            continue;
        }
        auto iter = devIter->second.find(pointerId);
...
bool InputWindowsManager::AdjustFingerFlag(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    auto iter = devIter->second.find(pointerEvent->GetPointerId());
    return (iter != devIter->second.end() && !(iter->second.flag));
}
...
int32_t InputWindowsManager::GetClientFd(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    if ((pointerEvent->GetSourceType() == PointerEvent::SOURCE_TYPE_MOUSE) ||
        ...
        if (mouseDownInfo_.agentPid != -1) {
            ...
            InitMouseDownInfo();
        } else if (pointerEvent->HasFlag(InputEvent::EVENT_FLAG_REDISPATCH)) {
            auto axisWindow = mouseRedispatchStore_.GetAxisBeginWindow();
            if (axisWindow && axisWindow->agentPid != -1) {
                agentPid = GetWindowAgentPid(axisWindow->agentWindowId);
                if (agentPid < 0) {
...
int32_t InputWindowsManager::GetDefaultGroupId() const
{
...
const OLD::DisplayGroupInfo* InputWindowsManager::GetDefaultGroupInfo() const
{
    for (const auto& it : displayGroupInfoMap_) {
        if (it.second.type == GroupType::GROUP_DEFAULT) {
            return &it.second;
...
int32_t InputWindowsManager::FindDisplayGroupId(int32_t displayId) const
{
    ...
    MMI_HILOG_DISPATCHD("DisplayId:%{public}d", displayId);
...
void InputWindowsManager::UpdateCaptureMode(const OLD::DisplayGroupInfo &displayGroupInfo)
{
    ...
    if (displayGroupInfo.windowsInfo.empty()) {
        MMI_HILOGW("The windowsinfo of synchronous groupId:%{public}d is empty", displayGroupInfo.groupId);
        return;
    }
    if (WindowInfo.empty()) {
        MMI_HILOGW("windowsInfo is empty, groupId:%{public}d", displayGroupInfo.groupId);
        return;
    }
    ...
    for (const auto &window : displayGroupInfo.windowsInfo) {
        if (window.windowType == static_cast<int32_t>(Rosen::WindowType::WINDOW_TYPE_TRANSPARENT_VIEW)) {
            ...
            if (auto touchGestureMgr = touchGestureMgr_.lock(); touchGestureMgr != nullptr) {
                touchGestureMgr->HandleGestureWindowEmerged(window.id, TouchLastPointerEventForGesture());
            }
            break;
        }
    }
...
CursorPosition InputWindowsManager::ResetCursorPos(const OLD::DisplayGroupInfo &displayGroupInfo)
{
    ...
    MMI_HILOGD("ResetCursorPos cursorPosMap_[groupId].displayId:%{public}d", cursorPosMap_[groupId].displayId);
...
{
    if (TouchLastPointerEventForGesture() == nullptr) {
        MMI_HILOGD("lastPointerEventForGesture_ is null");
        return;
    }
    if (TouchLastPointerEventForGesture()->GetSourceType() != PointerEvent::SOURCE_TYPE_TOUCHSCREEN) {
        MMI_HILOGD("source type:[%{public}d] is not touchscreen", TouchLastPointerEventForGesture()->GetSourceType());
        return;
    }
    int32_t touchDisplayId = TouchLastPointerEventForGesture()->GetTargetDisplayId();
    for (auto &currentDisplay : displayGroupInfo.displaysInfo) {
        ...
        if (touchDisplayId == currentDisplay.id && IsValidDisplayChange(currentDisplay)) {
            CancelAllTouches(TouchLastPointerEventForGesture(), true);
            return;
        }
    }
}
...
void InputWindowsManager::HandleWindowPositionChange(const OLD::DisplayGroupInfo &displayGroupInfo)
{
    ...
    for (auto it = touchItemDownInfosMap_[groupId].begin(); it != touchItemDownInfosMap_[groupId].end(); ++it) {
        ...
        if (iter != WindowInfo.end()) {
            ...
            auto tmpEvent = std::make_shared<PointerEvent>(*TouchLastPointerEventForWindowChangeMap()[groupId]);
...
void InputWindowsManager::SendCancelEventWhenWindowChange(int32_t pointerId, int32_t groupId)
{
    ...
    auto& windowChangeMap = TouchLastPointerEventForWindowChangeMap();
    const auto iter = windowChangeMap.find(groupId);
    if (iter != windowChangeMap.end()) {
...
bool InputWindowsManager::JudgeCameraInFore()
{
    CALL_DEBUG_ENTER;
    int32_t defaultGroupId = GetDefaultGroupId();
    int32_t focWid = GetFocusWindowId(defaultGroupId);
    int mainDisplayId = GetMainDisplayId(defaultGroupId);
...
void InputWindowsManager::InitDisplayGroupInfo(OLD::DisplayGroupInfo &displayGroupInfo)
{
    ...
    if (displayGroupInfo.type == GroupType::GROUP_DEFAULT) {
        if (groupId != MAIN_GROUPID) {
            MMI_HILOGI("GROUP_DEFAULT with groupId:%{public}d", groupId);
        }
    }
...
#endif // OHOS_BUILD_ENABLE_ANCO
    MMI_HILOGD("Displays Info size:%{public}zu, focusWindowId:%{public}d, winNum:%{public}zu",
        displayGroupInfo.displaysInfo.size(), displayGroupInfo.focusWindowId, displayGroupInfo.windowsInfo.size());
...
void InputWindowsManager::AdjustDisplayRotation(int32_t groupId)
{
    ...
    if (cursorPosCur.displayDirection != displayInfo->displayDirection ||
        ...
        MMI_HILOGD("displayId:%{public}d, cursorPosX:%{private}.2f, cursorPosY:%{private}.2f, direction:%{public}d, "
...
{
    int32_t defaultGroupId = GetDefaultGroupId();
    const auto iter = displayModeMap_.find(defaultGroupId);
    ...
    bool isDisplayChanged)
{
    if (!INPUT_DEV_MGR->HasPointerDeviceIncludingVirtual()) {
        ...
        int32_t defaultGroupId = GetDefaultGroupId();
        NotifyPointerToWindow(defaultGroupId);
        return;
    }
...
void InputWindowsManager::SendPointerEvent(int32_t pointerAction)
{
    ...
    if (pointerAction == PointerEvent::POINTER_ACTION_ENTER_WINDOW ||
        ...
        auto touchWindow = GetWindowInfo(lastLogicX_, lastLogicY_, GetDefaultGroupId());
        if (!touchWindow) {
...
void InputWindowsManager::DispatchPointer(int32_t pointerAction, int32_t windowId)
{
    ...
    if (pointerAction == PointerEvent::POINTER_ACTION_ENTER_WINDOW && windowId <= 0) {
        ...
        if (checkFlag) {
            windowInfo = GetWindowInfo(lastLogicX_, lastLogicY_, GetDefaultGroupId());
        } else {
    ...
    currentPointerItem.SetWindowXPos(windowXY.first);
    currentPointerItem.SetWindowYPos(windowXY.second);
    if (pointerAction == PointerEvent::POINTER_ACTION_ENTER_WINDOW && windowId > 0) {
        ...
        const auto iter = mouseLocationMap_.find(GetDefaultGroupId());
        if (iter != mouseLocationMap_.end()) {
            displayId = iter->second.displayId;
...
void InputWindowsManager::DispatchPointerRedispatch(int32_t pointerAction, const WindowInfo& windowInfo)
{
    ...
    LogTracer lt(pointerEvent->GetId(), pointerEvent->GetEventType(), pointerAction);
    PointerEvent::PointerItem lastPointerItem;
    int32_t lastPointerId = lastPointerEvent->GetPointerId();
    ...
    pointerEvent->SetPointerAction(pointerAction);
    pointerEvent->SetSourceType(PointerEvent::SOURCE_TYPE_MOUSE);
    pointerEvent->SetButtonId(lastPointerEvent->GetButtonId());
    int64_t time = GetSysClockTime();
    pointerEvent->SetActionTime(time);
    pointerEvent->SetActionStartTime(time);
    pointerEvent->ClearButtonPressed();
    std::set<int32_t> pressedButtons = lastPointerEvent->GetPressedButtons();
    for (const auto &buttonId : pressedButtons) {
        pointerEvent->SetButtonPressed(buttonId);
    }
    int32_t axisEventType = lastPointerEvent->GetAxisEventType();
    pointerEvent->SetAxisEventType(axisEventType);
    std::vector<int32_t> pressedKeys = lastPointerEvent->GetPressedKeys();
...
void InputWindowsManager::NotifyPointerToWindow(int32_t groupId)
{
    ...
    if ((lastPointerEventCopy->GetSourceType() == PointerEvent::SOURCE_TYPE_MOUSE) &&
        (!lastPointerEventCopy->GetPressedButtons().empty() ||
        lastPointerEventCopy->GetPointerAction() == PointerEvent::POINTER_ACTION_AXIS_BEGIN ||
        lastPointerEventCopy->GetPointerAction() == PointerEvent::POINTER_ACTION_AXIS_UPDATE ||
        lastPointerEventCopy->GetPointerAction() == PointerEvent::POINTER_ACTION_AXIS_END)) {
...
void InputWindowsManager::PrintDisplayGroupInfo(const OLD::DisplayGroupInfo displayGroupInfo)
{
    ...
    MMI_HILOGD("logicalInfo,focusWindowId:%{public}d,"
               "windowsInfosNum:%{public}zu,displayInfosNum:%{public}zu, groupId:%{public}d",
        displayGroupInfo.focusWindowId,
        displayGroupInfo.windowsInfo.size(),
        displayGroupInfo.displaysInfo.size(), displayGroupInfo.groupId);
...
void InputWindowsManager::TriggerTouchUpOnInvalidAreaEntry(int32_t pointerId)
{
    ...
    PointerEvent::PointerItem item;
    if (!(TouchLastPointerEventForGesture()->GetPointerItem(pointerId, item))) {
        MMI_HILOGE("Get pointer item failed, pointerId:%{public}d", pointerId);
        return;
    }
    // Make sure to trigger touch up the first time out of the valid area
    if ((!item.IsCanceled()) && item.IsPressed()) {
        auto pointerEvent = std::make_shared<PointerEvent>(*TouchLastPointerEventForGesture());
        ...
        item.SetCanceled(true);
        TouchLastPointerEventForGesture()->UpdatePointerItem(pointerId, item);
    }
}
...
const std::vector<OLD::DisplayInfo>& InputWindowsManager::GetDisplayInfoVector(int32_t groupId) const
{
    ...
    auto defaultGroup = GetDefaultGroupInfo();
    if (defaultGroup != nullptr) {
        return defaultGroup->displaysInfo;
    }
...
const std::vector<WindowInfo>& InputWindowsManager::GetWindowInfoVector(int32_t groupId) const
{
    ...
    auto defaultGroup = GetDefaultGroupInfo();
    if (defaultGroup != nullptr) {
        return defaultGroup->windowsInfo;
    }
...
int32_t InputWindowsManager::GetFocusWindowId(int32_t groupId) const
{
    ...
    auto defaultGroup = GetDefaultGroupInfo();
    if (defaultGroup != nullptr) {
        return defaultGroup->focusWindowId;
    }
    return 0;
}
    ...
    for (auto &item : pointerStyle_) {
        for (auto &innerIt : item.second) {
            if (innerIt.first != windowId && innerIt.second.id == MOUSE_ICON::DEVELOPER_DEFINED_ICON) {
                MMI_HILOG_CURSORD("pid=%{public}d, windowid:%{public}d, pointer style: %{public}d -> %{public}d",
                    item.first, innerIt.first, innerIt.second.id, MOUSE_ICON::DEFAULT);
    ...
    const sptr<IRemoteObject> &token)
{
    ...
    if (windowId == GLOBAL_WINDOW_ID) {
        ...
        MMI_HILOG_CURSORD("SetPointerStyle, setting global pointer style:%{public}d", globalStyle_.id);
        return RET_OK;
    }

    auto [isCheckOk, hostPid, hostWindowId] = GetHostPidAndHostWindowId(pid, windowId, token);
    if (!isCheckOk) {
        MMI_HILOG_CURSORI("SetPointerStyle, failed to get host pid and host windowId");
        return RET_ERR;
    }
    MMI_HILOG_CURSORD("SetPointerStyle, (%{public}d, %{public}d, %{public}d) => [%{public}d][%{public}d]=%{public}d",
        pid, windowId, token != nullptr ? 1 : 0, hostPid, hostWindowId, pointerStyle.id);
    return SetPointerStyleInfo(hostPid, hostWindowId, pointerStyle);
}

std::tuple<bool, int32_t, int32_t> InputWindowsManager::GetHostPidAndHostWindowId(int32_t pid, int32_t windowId,
    const sptr<IRemoteObject> &token) const
{
    if (token == nullptr) {
        return {true, pid, windowId};
    }

    auto uiExtensionInfoIter = std::find_if(uiExtensionInfos_.begin(), uiExtensionInfos_.end(),
        [pid, windowId, token] (const UIExtensionInfo &info) {
            return pid == info.pid && windowId == info.hostWindowId && token == info.token;
        });

    if (uiExtensionInfoIter == uiExtensionInfos_.end()) {
        MMI_HILOG_CURSORI("Not found UEA Info, ueaPid:%{public}d, hostWindowId:%{public}d", pid, windowId);
        return {false, INVALID_PID, -1};
    }

    int32_t hostPid = GetWindowPid(windowId);
    if (hostPid < 0) {
        MMI_HILOG_CURSORI("Failed to get hostPid, ueaPid:%{public}d, hostWindowId:%{public}d", pid, windowId);
        return {false, INVALID_PID, -1};
    }

    return {true, hostPid, windowId};
}

int32_t InputWindowsManager::SetPointerStyleInfo(int32_t pid, int32_t windowId, const PointerStyle &pointerStyle)
{
    CALL_DEBUG_ENTER;
    auto &windowMap = pointerStyle_[pid];
    auto iter = windowMap.find(windowId);
    if (iter == windowMap.end()) {
        if (windowMap.size() > POINTER_STYLE_WINDOW_NUM) {
            MMI_HILOG_CURSORW("the number of pointer styles of process:%{public}d exceeds %{public}zu",
                pid, POINTER_STYLE_WINDOW_NUM);
            windowMap.clear();
        }
        windowMap[windowId] = pointerStyle;
        MMI_HILOG_CURSORI("SetPointerStyle, add [%{public}d][%{public}d]=%{public}d", pid, windowId, pointerStyle.id);
    } else if (!(iter->second == pointerStyle)) {
        iter->second = pointerStyle;
        MMI_HILOG_CURSORI("SetPointerStyle, update [%{public}d][%{public}d]=%{public}d", pid, windowId,
            pointerStyle.id);
    } else {
        MMI_HILOG_CURSORD("SetPointerStyle, no changed [%{public}d][%{public}d]=%{public}d", pid, windowId,
            pointerStyle.id);
    }
    ...
    const sptr<IRemoteObject> &token) const
{
    ...
    if (!isCheckOk) {
        pointerStyle = globalStyle_;
        MMI_HILOG_CURSORI("GetPointerStyle, failed to get host pid and host windowId");
        return RET_OK;
    }
    return GetPointerStyleInfo(hostPid, hostWindowId, pointerStyle);
}

int32_t InputWindowsManager::GetPointerStyleInfo(int32_t pid, int32_t windowId, PointerStyle &pointerStyle) const
{
    CALL_DEBUG_ENTER;
    auto windowMapIter = pointerStyle_.find(pid);
    if (windowMapIter == pointerStyle_.end()) {
        pointerStyle = globalStyle_;
        MMI_HILOG_CURSORE("GetPointerStyle, the pointer style map is not include param pd, %{public}d", pid);
        return RET_OK;
    }

    const auto &windowMap = windowMapIter->second;
    auto styleIter = windowMap.find(windowId);
    if (styleIter == windowMap.end()) {
        pointerStyle = globalStyle_;
        MMI_HILOG_CURSORD("GetPointerStyle, windowId:%{public}d not found, using global style:%{public}d",
            windowId, pointerStyle.id);
        return RET_OK;
    }

    const auto &style = styleIter->second;
    if (style.id == DEFAULT_POINTER_STYLE) {
        pointerStyle = globalStyle_;
    } else {
        pointerStyle = style;
    }
    MMI_HILOG_CURSORD("GetPointerStyle, pointer style found, pid=%{public}d, windowId:%{public}d, style:%{public}d",
        pid, windowId, pointerStyle.id);
    return RET_OK;
}
#endif // OHOS_BUILD_ENABLE_POINTER || OHOS_BUILD_ENABLE_TOUCH
    ...
    const std::shared_ptr<PointerEvent>& pointerEvent)
{
    ...
    auto firstBtnDownWindowInfo = firstBtnDownWindowInfo_;
    int32_t action = pointerEvent->GetPointerAction();
    bool dispatchEventFlag = mouseRedispatchStore_.IsActive();
    bool checkFlag = dispatchEventFlag
        ? (action == PointerEvent::POINTER_ACTION_AXIS_BEGIN) && (pointerEvent->GetPressedButtons().empty())
    ...
    if (checkFlag) {
        ...
        for (const auto &item : windowsInfo) {
            ...
            if (IsAccessibilityEventWithZorderInjected(pointerEvent) && pointerEvent->GetZOrder() <= item.zOrder) {
                ...
                if (IsInHotArea(logicalX, logicalY, item.pointerHotAreas, item)) {
                    ...
                    if (IsInHotArea(logicalX, logicalY, item.dragDisabledAreas, item)) {
                        winId2ZorderMap.insert({item.id, item.zOrder});
                        continue;
                    }
                    firstBtnDownWindowInfo.first = item.id;
                    firstBtnDownWindowInfo.second = item.displayId;
                    MMI_HILOG_DISPATCHD("Mouse event select pull window, window:%{public}d, pid:%{public}d",
                        firstBtnDownWindowInfo.first, item.pid);
                    break;
                } else {
                    winId2ZorderMap.insert({item.id, item.zOrder});
                    continue;
                ...
                if ((item.flags & WindowInputPolicy::FLAG_MOUSE_LEFT_BUTTON_LOCK) ==
                    ...
                    !(IsTouchPadScrollAxis(item, pointerEvent))) {
                ...
                if (!item.uiExtentionWindowInfo.empty()) {
                    ...
                    CheckUIExtentionWindowPointerHotArea(logicalX, logicalY,
                        item.uiExtentionWindowInfo, firstBtnDownWindowInfo.first);
                }
                ...
                firstBtnDownWindowInfo.first = targetWindowId;
                firstBtnDownWindowInfo.second = item.displayId;
                MMI_HILOG_DISPATCHD("Find out the dispatch window of this pointer event when the targetWindowId "
        ...
        if ((firstBtnDownWindowInfo.first < 0) && (action == PointerEvent::POINTER_ACTION_BUTTON_DOWN) &&
    ...
    if (dispatchEventFlag) {
        if (action == PointerEvent::POINTER_ACTION_AXIS_UPDATE || action == PointerEvent::POINTER_ACTION_AXIS_END) {
            auto axisBeginWindow = mouseRedispatchStore_.GetAxisBeginWindow();
            if (axisBeginWindow) {
                firstBtnDownWindowInfo = {axisBeginWindow->id, axisBeginWindow->displayId};
            } else {
    ...
    MMI_HILOG_DISPATCHD("firstBtnDownWindowInfo.first:%{public}d", firstBtnDownWindowInfo.first);
#ifdef OHOS_BUILD_PC_PRIORITY
    PrintHighZorder(windowsInfo, pointerEvent->GetPointerAction(), firstBtnDownWindowInfo.first, logicalX, logicalY);
#endif // OHOS_BUILD_PC_PRIORITY
    for (const auto &item : windowsInfo) {
        for (const auto &windowInfo : item.uiExtentionWindowInfo) {
            if (windowInfo.id == firstBtnDownWindowInfo.first) {
                firstBtnDownWindowInfo.second = pointerEvent->GetTargetDisplayId();
                return std::make_optional(windowInfo);
            }
        }
        if (item.id == firstBtnDownWindowInfo.first) {
            firstBtnDownWindowInfo.second = pointerEvent->GetTargetDisplayId();
            return std::make_optional(item);
        }
    }
    if (pointerEvent->GetTargetDisplayId() != firstBtnDownWindowInfo.second) {
        std::vector<WindowInfo> firstBtnDownWindowsInfo =
            GetWindowGroupInfoByDisplayId(firstBtnDownWindowInfo.second);
        for (const auto &item : firstBtnDownWindowsInfo) {
            for (const auto &windowInfo : item.uiExtentionWindowInfo) {
                if (windowInfo.id == firstBtnDownWindowInfo.first) {
                    return std::make_optional(windowInfo);
...
void InputWindowsManager::UpdatePointerChangeAreas()
{
    CALL_DEBUG_ENTER;
    if (Rosen::SceneBoardJudgement::IsSceneBoardEnabled()) {
        const auto iter = displayGroupInfoMapTmp_.find(GetDefaultGroupId());
    ...
    const std::shared_ptr<PointerEvent>& pointerEvent, const WindowInfo& touchWindow)
{
    ...
    if (pointerEvent->HasFlag(InputEvent::EVENT_FLAG_REDISPATCH)) {
        {
            std::lock_guard<std::mutex> guard(mtx_);
            mouseRedispatchStore_.CacheLastEvent(pointerEvent);
        }
        return;
    }
...
int32_t InputWindowsManager::UpdateMouseTarget(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_AXIS_BEGIN) {
        if (dispatchEventFlag) {
            mouseRedispatchStore_.SetAxisBeginWindow(touchWindow);
            if (touchWindow) {
            ...
            if (axisBeginWindowInfo_) {
                ...
                for (float key : keysToErase) {
                    mouseRedispatchStore_.EraseAxisBeginWindow(key);
                }
            }
        }
    }
    ...
    PointerStyle pointerStyle;
    PointerStyle pointerOriginStyle;
#ifdef OHOS_BUILD_ENABLE_POINTER_DRAWING
    if (Rosen::SceneBoardJudgement::IsSceneBoardEnabled()) {
...
#endif
    GetPointerStyle(touchWindow->pid, touchWindow->id, pointerStyle);
    pointerOriginStyle = dragPointerStyle_;
    if (!isDragBorder_) {
        GetPointerStyle(touchWindow->pid, touchWindow->id, pointerStyle);
        dragPointerStyle_ = pointerStyle;
    }
    ...
    MMI_HILOGD("pid:%{public}d, id:%{public}d, agentWindowId:%{public}d,"
        ...
        touchWindow->pid, touchWindow->id, touchWindow->agentWindowId, logicalX, logicalY,
...
int32_t InputWindowsManager::SetMouseCaptureMode(int32_t windowId, bool isCaptureMode)
{
    ...
    int32_t defaultGroupId = GetDefaultGroupId();
    auto itr = captureModeInfoMap_.find(defaultGroupId);
    if (itr != captureModeInfoMap_.end()) {
        if (itr->second.isCaptureMode == isCaptureMode && !isCaptureMode) {
            MMI_HILOGE("Windowid:(%{public}d) is not capture mode", windowId);
            return RET_OK;
        }
        captureModeInfoMap_[defaultGroupId].windowId = windowId;
        captureModeInfoMap_[defaultGroupId].isCaptureMode = isCaptureMode;
    }
...
{
    CALL_DEBUG_ENTER;
    auto itr = captureModeInfoMap_.find(GetDefaultGroupId());
    if (itr != captureModeInfoMap_.end()) {
...
void InputWindowsManager::ClearPointerDeviceId(const std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_UP) {
        ...
        if (pointerItem.GetToolType() == PointerEvent::TOOL_TYPE_THP_FEATURE) {
            ...
            ErasePointerDeviceId(pointerEvent, TouchItemDownInfos());
        }
    }
    return;
}

std::map<int32_t, std::map<int32_t, std::set<int32_t>>>& InputWindowsManager::TouchTargetWinIds()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetTargetTouchWinIds() : targetTouchWinIds_;
}

std::map<int32_t, WindowPartInfo>& InputWindowsManager::TouchFirstTouchInfos()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetFirstTouchInfos() : firstTouchWindowInfos_;
}

WindowInfo& InputWindowsManager::TouchLockWindowInfo()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetLockWindowInfo() : lockWindowInfo_;
}

std::shared_ptr<PointerEvent>& InputWindowsManager::TouchLastTouchEventOnBackGesture()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetLastTouchEventOnBackGesture() : lastTouchEventOnBackGesture_;
}

std::map<int32_t, std::shared_ptr<PointerEvent>>& InputWindowsManager::TouchLastPointerEventForWindowChangeMap()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetLastPointerEventForWindowChangeMap() : lastPointerEventForWindowChangeMap_;
}

std::shared_ptr<PointerEvent>& InputWindowsManager::TouchLastPointerEventForGesture()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetLastPointerEventForGesture() : lastPointerEventForGesture_;
}

PointerDispatchEventCache& InputWindowsManager::TouchDispatchEventCache()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetDispatchEventCache() : dispatchEventCache_;
}

std::map<LastTouch, LastTouchInfo>& InputWindowsManager::TouchLastTouchInfos()
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetLastTouchInfos() : LastTouchInfos_;
}

std::map<int32_t, std::vector<std::shared_ptr<WindowInfo>>>& InputWindowsManager::GetCancelEventList(
    std::map<int32_t, std::vector<std::shared_ptr<WindowInfo>>>& realList)
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetCancelEventList() : realList;
}

bool InputWindowsManager::AbandonTouchRedispatch(const std::shared_ptr<PointerEvent>& pointerEvent)
{
    return touchRedispatchStore_.Abandon(pointerEvent);
}

bool InputWindowsManager::AbandonMouseRedispatch(const std::shared_ptr<PointerEvent>& pointerEvent)
{
    return mouseRedispatchStore_.Abandon(pointerEvent);
}

MouseRedispatchStore& InputWindowsManager::GetMouseRedispatchStore()
{
    return mouseRedispatchStore_;
}

TouchRedispatchStore& InputWindowsManager::GetTouchRedispatchStore()
{
...
{
    return touchRedispatchStore_.IsActive()
        ? touchRedispatchStore_.GetTouchItemDownInfos() : touchItemDownInfos_;
}
...
int32_t InputWindowsManager::UpdateTouchScreenTarget(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    for (auto &item : windowsInfo) {
        ...
        if (ShouldIgnoreLevitateEvent(pointerItem, item.windowInputType)) {
            winMap.insert({item.id, item});
            continue;
        }
        ...
        bool isSlidTouch = (pointerItem.GetToolType() == PointerEvent::TOOL_TYPE_FINGER  &&
            ...
            (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_PULL_UP);
        if (isSlidTouch && TouchLockWindowInfo().windowInputType == WindowInputType::SLID_TOUCH_WINDOW) {
            if (IsInHotArea(static_cast<int32_t>(logicalX), static_cast<int32_t>(logicalY),
                item.defaultHotAreas, item)) {
                UpdateTargetTouchWinIds(item, pointerItem, pointerEvent, pointerId, displayId,
                    pointerEvent->GetDeviceId());
        ...
        if (targetWindowId >= 0 && pointerEvent->GetPointerAction() != PointerEvent::POINTER_ACTION_DOWN &&
            ...
            CheckUIExtentionWindowDefaultHotArea(logicalXY, isHotArea, pointerEvent, item.uiExtentionWindowInfo,
                &touchWindow);
            if (isSpecialWindow) {
                PrintSpecialWindow(pointerEvent->GetPointerAction(), item);
                AddTargetWindowIds(pointerEvent->GetPointerId(), pointerEvent->GetSourceType(), item.id,
                    pointerEvent->GetDeviceId());
                isHotArea = true;
                continue;
    ...
    std::map<int32_t, std::map<int32_t, WindowInfoEX>> tmpWindowInfo;
    if (pointerItem.GetToolType() == PointerEvent::TOOL_TYPE_THP_FEATURE) {
        tmpWindowInfo = thpFeatureTouchDownInfos_;
    } else {
        tmpWindowInfo = TouchItemDownInfos();
    }
    if (touchWindow == nullptr) {
        if (tmpWindowInfo.find(pointerEvent->GetDeviceId()) == tmpWindowInfo.end()) {
            MMI_HILOGI("get deviceId failed:%{public}d", pointerEvent->GetDeviceId());
            return RET_ERR;
        }
        ...
        if (pointerItem.GetToolType() == PointerEvent::TOOL_TYPE_THP_FEATURE) {
            if (it->second.flag) {
                thpFeatureTouchDownInfos_[pointerEvent->GetDeviceId()][pointerId].flag = false;
            } else {
                pointerItem.SetCanceled(true);
            }
        }
    ...
    if ((action == PointerEvent::POINTER_ACTION_DOWN ||
        ...
        !pointerEvent->HasFlag(InputEvent::EVENT_FLAG_REDISPATCH)) {
        for (const auto& [zOrder, fingerMap] : touchRedispatchStore_.GetFingerMap()) {
            auto iter = fingerMap.find({pointerEvent->GetDeviceId(), pointerId});
            if (iter != fingerMap.end() && iter->second.windowId == touchWindow->id) {
                MMI_HILOGI("Touch DOWN conflicts with active redispatch finger, "
                    "zOrder:%{public}f deviceId:%{public}d pointerId:%{public}d windowId:%{public}d",
                    zOrder, pointerEvent->GetDeviceId(), pointerId, touchWindow->id);
                DispatchTouchRedispatch(PointerEvent::POINTER_ACTION_CANCEL,
                    zOrder, pointerEvent->GetDeviceId(), pointerId, *touchWindow);
                touchRedispatchStore_.DeactivateFinger(zOrder, pointerEvent->GetDeviceId(), pointerId);
                break;
            }
        }
    }
    ...
    auto& ancoStates = isHoverEvent ? hoverAncoStates_ : touchAncoStates_;
    if ((isHoverEvent && (pointerAction == PointerEvent::POINTER_ACTION_HOVER_ENTER ||
            NeedTouchTracking(*pointerEvent))) ||
        (!isHoverEvent && (pointerAction == PointerEvent::POINTER_ACTION_DOWN || isExtraData))) {
        ancoStates[deviceId][pointerId] =
            touchWindow && IsInAncoWindow(*touchWindow, windowX, windowY);
    }
    bool isInAnco = GetAncoState(ancoStates, deviceId, pointerId);
    if ((isHoverEvent && pointerAction == PointerEvent::POINTER_ACTION_HOVER_EXIT) ||
        (!isHoverEvent && (pointerAction == PointerEvent::POINTER_ACTION_UP ||
        pointerAction == PointerEvent::POINTER_ACTION_CANCEL ||
        pointerAction == PointerEvent::POINTER_ACTION_PULL_CANCEL ||
        pointerAction == PointerEvent::POINTER_ACTION_PULL_UP))) {
        auto devIter = ancoStates.find(deviceId);
        if (devIter != ancoStates.end()) {
            devIter->second.erase(pointerId);
        }
    }
    if (isInAnco) {
    ...
    if (touchWindow->windowInputType == WindowInputType::MIX_LEFT_RIGHT_ANTI_AXIS_MOVE) {
        ...
        if (TouchLastTouchEventOnBackGesture() != nullptr &&
    ...
    TouchLastPointerEventForWindowChangeMap()[curGroupId] = pointerEvent;
    TouchLastPointerEventForGesture() = pointerEvent;
    deviceId = TouchLastPointerEventForGesture()->GetDeviceId();
    pointerAction = pointerEvent->GetPointerAction();
...
void InputWindowsManager::ProcessOtherTouchHit(std::shared_ptr<PointerEvent> pointerEvent,
    PointerEvent::PointerItem& pointerItem, const WindowInfo* touchWindow)
{
    ...
    if (pointerItem.GetPointerId() != currentPointerId) {
        MMI_HILOG_DISPATCHE("pointer id different, currentPointerId:%{public}d, itemId:%{public}d",
            currentPointerId, pointerItem.GetPointerId());
        return;
    }
    pointerItem.SetCanceled(true);
    if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_DOWN) {
        MMI_HILOG_DISPATCHI("currentPointerId:%{public}d set canceled", currentPointerId);
    }
    return;
}
    ...
    const PointerEvent::PointerItem& pointerItem)
{
    ...
    auto& firstInfos = TouchFirstTouchInfos();
    auto firstIter = firstInfos.find(deviceId);
    if (firstIter == firstInfos.end()) {
        return false;
    }
    auto& firstTouchInfo = firstIter->second;
    if (firstTouchInfo.flags & WindowInputPolicy::FLAG_FIRST_TOUCH_HIT) {
        return true;
    }
    return false;
}

bool InputWindowsManager::IsFindFirstTouchFlagWindow(const WindowInfo &item, std::shared_ptr<PointerEvent> pointerEvent)
{
    if (pointerEvent == nullptr) {
        return false;
    }
    int32_t deviceId = pointerEvent->GetDeviceId();
    auto& firstInfos2 = TouchFirstTouchInfos();
    auto firstIter2 = firstInfos2.find(deviceId);
    if (firstIter2 == firstInfos2.end()) {
        return false;
    }
    auto& firstTouchInfo = firstIter2->second;
    if (firstTouchInfo.windowId == item.id &&
        firstTouchInfo.displayId == item.displayId) {
        if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_DOWN) {
            MMI_HILOG_DISPATCHI("currentPointerId:%{public}d follow first touch window:%{public}d",
                pointerEvent->GetPointerId(), item.id);
        }
        return true;
    }
    for (const auto &uiExtentionWindow : item.uiExtentionWindowInfo) {
        if (firstTouchInfo.windowId == uiExtentionWindow.id &&
            firstTouchInfo.displayId == uiExtentionWindow.displayId) {
            if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_DOWN) {
                MMI_HILOG_DISPATCHI("currentPointerId:%{public}d follow first touch uiExtentionwindow:%{public}d",
                    pointerEvent->GetPointerId(), uiExtentionWindow.id);
            }
            return true;
        }
    }
    ...
    const PointerEvent::PointerItem& pointerItem, const WindowInfo* touchWindow)
{
    ...
    if (IsFirstTouch(pointerEvent->GetPointerId(), pointerEvent->GetPointerCount())) {
        if (pointerEvent->GetPointerAction() == PointerEvent::POINTER_ACTION_DOWN) {
            auto& firstInfo = TouchFirstTouchInfos()[deviceId];
            firstInfo.displayId = touchWindow->displayId;
            firstInfo.windowId = touchWindow->id;
            firstInfo.flags = touchWindow->flags;
            MMI_HILOG_DISPATCHD("TouchFirstTouchInfos():deviceId:%{public}d,PI:%{public}d,"
        ...
        if (checkExtraData) {
            auto& firstInfo2 = TouchFirstTouchInfos()[deviceId];
            firstInfo2.displayId = touchWindow->displayId;
            firstInfo2.windowId = touchWindow->id;
            firstInfo2.flags = touchWindow->flags;
            MMI_HILOG_DISPATCHD("TouchFirstTouchInfos() drag:deviceId:%{public}d,PI:%{public}d,"
                "WI:%{public}d, flags:%{public}d", deviceId, pointerEvent->GetPointerId(),
                touchWindow->id, touchWindow->flags);
            return;
        }
...
{
    CALL_DEBUG_ENTER;
    auto& firstTouchInfos = TouchFirstTouchInfos();
    if (firstTouchInfos.find(deviceId) == firstTouchInfos.end()) {
        MMI_HILOGD("TouchFirstTouchInfos() not found, deviceId:%{public}d", deviceId);
        return;
    }
    firstTouchInfos.erase(deviceId);
#ifdef OHOS_BUILD_ENABLE_ANCO
    touchAncoStates_.erase(deviceId);
    hoverAncoStates_.erase(deviceId);
#endif // OHOS_BUILD_ENABLE_ANCO
}
...
void InputWindowsManager::UpdateTargetTouchWinIds(const WindowInfo &item, PointerEvent::PointerItem &pointerItem,
    ...
    if ((item.flags & WindowInputPolicy::FLAG_EVENT_TRANSMIT_ALL) != WindowInputPolicy::FLAG_EVENT_TRANSMIT_ALL) {
        auto& targetWinIds = TouchTargetWinIds();
        auto twiIter = targetWinIds.find(deviceId);
        if (twiIter == targetWinIds.end()) {
            return;
        }
        auto& targetIds = twiIter->second;
        WIN_MGR->GetTargetWindowIds(pointerItem.GetPointerId(), pointerEvent->GetSourceType(),
            targetIds[pointerId], deviceId);
        if (!targetIds[pointerId].empty()) {
            ClearMismatchTypeWinIds(pointerId, displayId, deviceId);
            targetIds[pointerId].insert(item.id);
        }
    }
}
...
void InputWindowsManager::ClearMismatchTypeWinIds(int32_t pointerId, int32_t displayId, int32_t deviceId) {
    ...
    auto& targetIds = twiIter->second;
    if (targetIds.find(pointerId) == targetIds.end()) {
        return;
    }
    std::set<int32_t>& windowIds = targetIds[pointerId];
...
LastTouchInfo InputWindowsManager::GetLastTouchInfo(std::shared_ptr<PointerEvent> pointerEvent)
{
    LastTouchInfo lastInfo;
    if (pointerEvent == nullptr) {
        MMI_HILOG_DISPATCHW("pointerEvent is nullptr");
        return lastInfo;
    }
    LastTouch touch{
        .deviceId_ = pointerEvent->GetDeviceId(), .pointerId_ = pointerEvent->GetPointerId()};
    lastInfo.lastTouchLogicX = -1;
    lastInfo.lastTouchLogicY = -1;
    lastInfo.lastTouchWindowInfo.id = -1;
    lastInfo.lastTouchWindowInfo.pid = -1;
    lastInfo.lastTouchWindowInfo.uid = -1;
    lastInfo.lastTouchWindowInfo.agentWindowId = -1;
    lastInfo.lastTouchWindowInfo.area = { 0, 0, 0, 0 };
    lastInfo.lastTouchWindowInfo.flags = 0;
    lastInfo.lastTouchWindowInfo.windowType = 0;
    lastInfo.lastTouchWindowInfo.windowNameType = 0;
    auto& lastTouchInfos = TouchLastTouchInfos();
    auto iter = lastTouchInfos.find(touch);
    if (iter != lastTouchInfos.end()) {
...
bool InputWindowsManager::ShouldIgnoreLevitateEvent(const PointerEvent::PointerItem &pointerItem,
    WindowInputType windowType)
{
    if (pointerItem.GetToolType() == PointerEvent::TOOL_TYPE_PEN && MMI_LE(pointerItem.GetPressure(), 0.0f)) {
        return (windowType == WindowInputType::MIX_LEFT_RIGHT_ANTI_AXIS_MOVE) ||
...
void InputWindowsManager::UpdateStashTouchEventInfo(int32_t logicalX, int32_t logicalY,
    const std::shared_ptr<PointerEvent> pointerEvent, const WindowInfo* touchWindow)
{
    if (touchWindow == nullptr || pointerEvent == nullptr) {
        MMI_HILOG_DISPATCHW("TouchWindow or pointerEvent is nullptr");
        return;
    }
    LastTouch touch{
        .deviceId_ = pointerEvent->GetDeviceId(), .pointerId_ = pointerEvent->GetPointerId()};
...
void InputWindowsManager::DispatchTouch(int32_t pointerAction, int32_t groupId, int32_t toolType)
{
    ...
    if (pointerAction == PointerEvent::POINTER_ACTION_PULL_IN_WINDOW ||
        ...
        for (const auto &item : WindowsInfo) {
            ...
            if (IsInHotArea(lastInfo.lastTouchLogicX, lastInfo.lastTouchLogicY, item.dragDisabledAreas, item)) {
                continue;
            }
        ...
        if (touchWindow.id != lastInfo.lastTouchWindowInfo.id) {
            lastInfo.lastTouchWindowInfo = touchWindow;
        }
        if (pointerAction == PointerEvent::POINTER_ACTION_LEVITATE_IN_WINDOW) {
            lastLevitateInWindowInfo_ = touchWindow;
        }
    }
    auto pointerEvent = PointerEvent::Create();
    if (pointerEvent == nullptr) {
    ...
    if (pointerAction == PointerEvent::POINTER_ACTION_LEVITATE_OUT_WINDOW) {
        ...
        pointerEvent->SetTargetWindowId(lastLevitateInWindowInfo_.id);
    ...
    double& physicalY, int32_t& displayId)
{
    ...
    for (const auto &item : displaysInfoVector) {
        ...
        MMI_HILOGD("acrossDirection :%{public}d, current displayId:%{public}d, target displayId:%{public}d",
...
{
    int32_t defaultGroupId = GetDefaultGroupId();
    auto &displaysInfoVector = GetDisplayInfoVector(defaultGroupId);
    MouseLocation curMouseLocation;
    const auto iter = mouseLocationMap_.find(defaultGroupId);
    if (iter != mouseLocationMap_.end()) {
    ...
    if ((curMouseLocation.displayId < 0) && !displaysInfoVector.empty()) {
        ...
        const auto iter = mouseLocationMap_.find(defaultGroupId);
        if (iter != mouseLocationMap_.end()) {
            mouseLocationMap_[defaultGroupId].displayId = displayInfo.id;
            mouseLocationMap_[defaultGroupId].physicalX = displayInfo.validWidth / TWOFOLD;
            mouseLocationMap_[defaultGroupId].physicalY = displayInfo.validHeight / TWOFOLD;
            curMouseLocation = iter->second;
        }
...
CursorPosition InputWindowsManager::GetCursorPos()
{
    ...
    auto &displaysInfoVector = GetDisplayInfoVector(defaultGroupId);
    CursorPosition cursorPos;
    const auto iter = cursorPosMap_.find(defaultGroupId);
    if (iter != cursorPosMap_.end()) {
    ...
    if ((cursorPos.displayId < 0) && !displaysInfoVector.empty()) {
        ...
        if (iter != cursorPosMap_.end()) {
            ...
            cursorPosMap_[defaultGroupId].displayId = displayInfo.id;
            cursorPosMap_[defaultGroupId].cursorPos.x = validW * HALF_RATIO;
            cursorPosMap_[defaultGroupId].cursorPos.y = validH * HALF_RATIO;
            cursorPosMap_[defaultGroupId].direction = displayInfo.direction;
            cursorPosMap_[defaultGroupId].displayDirection = displayInfo.displayDirection;
            cursorPos = cursorPosMap_[defaultGroupId];
        }
    }
    return cursorPos;
}
...
CursorPosition InputWindowsManager::ResetCursorPos()
{
    ...
    if (!displaysInfoVector.empty()) {
        ...
        if (iter != cursorPosMap_.end()) {
            cursorPosMap_[defaultGroupId].displayId = displayInfo.id;
            cursorPosMap_[defaultGroupId].cursorPos.x = x;
            cursorPosMap_[defaultGroupId].cursorPos.y = y;
            cursorPosMap_[defaultGroupId].direction = displayInfo.direction;
            cursorPosMap_[defaultGroupId].displayDirection = displayInfo.displayDirection;
        }
    } else {
        const auto iter = cursorPosMap_.find(defaultGroupId);
        if (iter != cursorPosMap_.end()) {
            cursorPosMap_[defaultGroupId].displayId = -1;
            cursorPosMap_[defaultGroupId].cursorPos.x = 0;
            cursorPosMap_[defaultGroupId].cursorPos.y = 0;
        }
    }
    CursorPosition cursorPos;
    const auto iter = cursorPosMap_.find(defaultGroupId);
    ...
    MMI_HILOGD("ResetCursorPos cursorPosMap_[mainGroupId].displayId:%{public}d",
        cursorPos.displayId);
    return cursorPos;
}
#endif // OHOS_BUILD_ENABLE_POINTER || OHOS_BUILD_ENABLE_TOUCH
    ...
    const std::shared_ptr<PointerEvent>& pointerEvent)
{
    ...
    if (!(pointerEvent->HasFlag(InputEvent::EVENT_FLAG_SIMULATE) ||
            pointerEvent->HasFlag(InputEvent::EVENT_FLAG_REDISPATCH)) ||
        MMI_LE(pointerEvent->GetZOrder(), 0.0f)) {
        return true;
    }
    if (MMI_GE(window.zOrder, pointerEvent->GetZOrder())) {
        ...
        if (pointerAction != PointerEvent::POINTER_ACTION_MOVE &&
            pointerAction != PointerEvent::POINTER_ACTION_AXIS_UPDATE) {
            MMI_HILOGE("window zorder:%{public}f greater than the simulate target zOrder:%{public}f, "
                       "ignore this window::%{public}d",
                window.zOrder,
    ...
    std::set<int32_t> &windowIds, int32_t deviceId)
{
    ...
    if (sourceType == PointerEvent::SOURCE_TYPE_MOUSE) {
        ...
        auto& targetIds = twiIter->second;
        if (targetIds.find(pointerItemId) != targetIds.end()) {
                windowIds = targetIds[pointerItemId];
        }
    }
}
    ...
    int32_t deviceId)
{
    ...
    if (sourceType == PointerEvent::SOURCE_TYPE_MOUSE) {
        ...
        if (twiIter == targetWinIds.end()) {
            ...
            targetWinIds[deviceId] = {};
        }
        auto& targetIds = targetWinIds[deviceId];
        if (targetIds.find(pointerItemId) != targetIds.end()) {
            targetIds[pointerItemId].insert(windowId);
        } else {
            std::set<int32_t> windowIds;
            windowIds.insert(windowId);
            targetIds.emplace(pointerItemId, windowIds);
        }
    }
}
...
void InputWindowsManager::ClearTargetDeviceWindowId(int32_t deviceId)
{
    CALL_DEBUG_ENTER;
    auto& targetWinIds = TouchTargetWinIds();
    if (targetWinIds.find(deviceId) == targetWinIds.end()) {
        MMI_HILOGD("Target device's windowId not found, deviceId:%{public}d", deviceId);
        return;
    }
    targetWinIds.erase(deviceId);
}
...
{
    CALL_DEBUG_ENTER;
    auto &DisplaysInfo = GetDisplayInfoVector(GetDefaultGroupId());
    if (DisplaysInfo.empty()) {
        MMI_HILOGE("DisplaysInfo is empty");
        return;
    }
...
void InputWindowsManager::PrintChangedWindowBySync(const OLD::DisplayGroupInfo &newDisplayInfo)
{
    ...
    if (!oldWindows.empty() && !newWindows.empty()) {
        if (oldWindows[0].id != newWindows[0].id) {
            MMI_HILOGD("Window sync changed %{public}d %{public}d %{public}f %{public}d %{public}d %{public}f",
...
bool InputWindowsManager::GetAncoState(const std::map<int32_t, std::map<int32_t, bool>>& states,
    int32_t deviceId, int32_t pointerId) const
{
    auto devIter = states.find(deviceId);
    if (devIter == states.end()) {
        return false;
    }
    auto ptrIter = devIter->second.find(pointerId);
    return (ptrIter != devIter->second.end()) && (ptrIter->second);
}
#endif // OHOS_BUILD_ENABLE_ANCO
    ...
    const OLD::DisplayGroupInfo &newGroupInfo, bool hasOldGroupInfo) const
{
    ...
    for (const auto &oldDisplay : oldGroupInfo.displaysInfo) {
        ...
        if (newDisplay == nullptr) {
            MMI_HILOGD("Pointer back center triggered, reason:DISPLAY_REMOVED_OR_REPLACED, "
    ...
    MMI_HILOGD("No condition matched, no need center");
    return false;
}
...
{
    if (oldGroupInfo.mainDisplayId != newGroupInfo.mainDisplayId) {
        MMI_HILOGD("Pointer back center triggered, reason:MAIN_DISPLAY_ID_CHANGED, old:%{public}d, new:%{public}d",
            oldGroupInfo.mainDisplayId, newGroupInfo.mainDisplayId);
        return true;
    }
    ...
    if (GetMainScreenDisplayInfo(oldGroupInfo.displaysInfo, oldMainDisplayInfo) != RET_OK ||
        ...
        MMI_HILOGD("Pointer back center triggered, reason:MAIN_DISPLAY_INFO_UNAVAILABLE");
        return true;
    }

    if (oldMainDisplayInfo.id != newMainDisplayInfo.id ||
        oldMainDisplayInfo.displaySourceMode != newMainDisplayInfo.displaySourceMode) {
        MMI_HILOGD("Pointer back center triggered, reason:MAIN_DISPLAY_CHANGED, "
    ...
    const OLD::DisplayInfo &newDisplay) const
{
    if (oldDisplay.displayDirection != newDisplay.displayDirection) {
        MMI_HILOGD("Pointer back center triggered, reason:DISPLAY_DIRECTION_CHANGED, "
    ...
    const OLD::DisplayInfo &newDisplay) const
{
    if (oldDisplay.validWidth != newDisplay.validWidth ||
        ...
        oldDisplay.width != newDisplay.width ||
        oldDisplay.height != newDisplay.height) {
        MMI_HILOGD("Pointer back center triggered, reason:DISPLAY_RESOLUTION_CHANGED, "
    ...
    const OLD::DisplayInfo &newDisplay) const
{
    if (oldDisplay.x != newDisplay.x ||
        ...
        oldDisplay.offsetX != newDisplay.offsetX ||
        oldDisplay.offsetY != newDisplay.offsetY) {
        MMI_HILOGD("Pointer back center triggered, reason:DISPLAY_LAYOUT_CHANGED, "
    ...
    const OLD::DisplayInfo &newDisplay) const
{
    ...
    if (oldDisplay.displaySourceMode != newDisplay.displaySourceMode) {
        MMI_HILOGD("Pointer back center triggered, reason:DISPLAY_SOURCE_MODE_CHANGED, "
            "displayId:%{public}d, old:%{public}d, new:%{public}d",
...
{
    auto touchEvent = TouchDispatchEventCache().GetTouchEvent();
    ...
    item.SetWindowX(lastInfo.lastTouchLogicX - sourceWindowInfo.area.x);
    item.SetWindowY(lastInfo.lastTouchLogicY - sourceWindowInfo.area.y);
    item.SetWindowXPos(lastInfo.lastTouchLogicX - sourceWindowInfo.area.x);
    item.SetWindowYPos(lastInfo.lastTouchLogicY - sourceWindowInfo.area.y);
    item.SetPressed(false);
    ...
    item.SetWindowYPos(shiftWindowInfo.y);
    if (shiftWindowInfo.x == -1 && shiftWindowInfo.y == -1) {
        item.SetWindowX(lastInfo.lastTouchLogicX - targetWindowInfo.area.x);
        item.SetWindowY(lastInfo.lastTouchLogicY - targetWindowInfo.area.y);
        item.SetWindowXPos(lastInfo.lastTouchLogicX - targetWindowInfo.area.x);
        item.SetWindowYPos(lastInfo.lastTouchLogicY - targetWindowInfo.area.y);
    }
    item.SetPressed(true);
...
#if defined(OHOS_BUILD_ENABLE_TOUCH) && defined(OHOS_BUILD_ENABLE_MONITOR)
bool InputWindowsManager::CancelTouch(int32_t touch, int32_t deviceId)
{
    auto& infos = TouchItemDownInfos();
    auto devIter = infos.find(deviceId);
    if (devIter == infos.end()) {
    ...
    auto iter = devIter->second.find(touch);
    if ((iter != devIter->second.end()) && iter->second.flag) {
        iter->second.flag = false;
        return true;
    }
    return false;
...
void InputWindowsManager::CancelAllTouches(std::shared_ptr<PointerEvent> event, bool isDisplayChanged)
{
    ...
    for (const auto &item : items) {
        ...
        CancelTouch(item.GetPointerId(), item.GetDeviceId());
    }
}
    ...
    const std::shared_ptr<PointerEvent> pointerEvent, const WindowInfo* touchWindow)
{
    ...
    if (lastInfo.lastTouchWindowInfo.id != touchWindow->id) {
        if (lastInfo.lastTouchWindowInfo.id != -1 &&
            lastInfo.lastTouchWindowInfo.windowInputType == WindowInputType::SLID_TOUCH_WINDOW) {
...
void InputWindowsManager::EnterMouseCaptureMode(const OLD::DisplayGroupInfo &displayGroupInfo)
{
    ...
    if (mouseIt == mouseLocationMap_.end()) {
        ...
        mouseLocationMap_[groupId].physicalX = windowArea.x + windowArea.width / 2;
        mouseLocationMap_[groupId].physicalY = windowArea.y + windowArea.height / 2;
    ...
    if (IsInHotArea(
        ...
        if (pointerEvent != nullptr) {
            ...
            if (pointerLocked) {
                pointerLockedCursorPos_.x = windowArea.x + windowArea.width / 2;
                pointerLockedCursorPos_.y = windowArea.y + windowArea.height / 2;
            }
            if (pointerConfined && fistLockedWindowId) {
                cursorIt->second.cursorPos.x = windowArea.x + windowArea.width / 2;
                cursorIt->second.cursorPos.y = windowArea.y + windowArea.height / 2;
            }
...
void InputWindowsManager::RotateWindowArea(int32_t displayId, WindowInfo &window, Rect &windowArea)
{
    ...
    switch (displayDirection) {
        ...
        case DIRECTION90:
            ...
            windowArea.y = displayInfo->validWidth - temp - windowArea.width;
            temp = windowArea.height;
            windowArea.height = windowArea.width;
            windowArea.width = temp;
            break;
        case DIRECTION180:
            windowArea.x = displayInfo->validWidth - windowArea.x - windowArea.width;
            windowArea.y = displayInfo->validHeight - windowArea.y - windowArea.height;
            break;
        case DIRECTION270:
            temp = windowArea.x;
            windowArea.x = displayInfo->validHeight - windowArea.y - windowArea.height;
            windowArea.y = temp;
            temp = windowArea.height;
            windowArea.height = windowArea.width;
            windowArea.width = temp;
...
bool InputWindowsManager::IsTouchPadScrollAxis(
    const WindowInfo &window, const std::shared_ptr<PointerEvent> pointerEvent)
{
    CALL_DEBUG_ENTER;
    if ((window.flags & WindowInputPolicy::FLAG_TOUCHPAD_AXIS_SCROLL_REDISPATCH) !=
                    WindowInputPolicy::FLAG_TOUCHPAD_AXIS_SCROLL_REDISPATCH) {
        return false;
    }
    if (pointerEvent->GetAxisEventType() != PointerEvent::AXIS_EVENT_TYPE_SCROLL ||
        pointerEvent->GetSourceType() != PointerEvent::SOURCE_TYPE_MOUSE) {
        return false;
    }
    int32_t action = pointerEvent->GetPointerAction();
    ...
    PointerEvent::PointerItem pointerItem;
    if (!pointerEvent->GetPointerItem(pointerEvent->GetPointerId(), pointerItem)) {
        MMI_HILOGE("pointerItem is null");
        return false;
    }
    if (pointerItem.GetToolType() != PointerEvent::TOOL_TYPE_TOUCHPAD) {
        return false;
    }
    return true;
}

void InputWindowsManager::UpdateUIExtensionInfo(const std::vector<UIExtensionInfo> &uiExtensionInfos)
{
    uiExtensionInfos_ = uiExtensionInfos;
    MMI_HILOGD("update uiextension info, size:%{public}zu", uiExtensionInfos_.size());
}
} // namespace MMI
} // namespace OHOS
