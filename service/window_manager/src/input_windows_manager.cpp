...
{
    int32_t focusWindowId = GetFocusWindowId();
    return ((session >= 0) && (session == GetWindowPid(focusWindowId)));
}
...
int32_t InputWindowsManager::GetMainScreenDisplayInfo(const std::vector<DisplayInfo> &displaysInfo,
...
CursorPosition  InputWindowsManager::ResetCursorPos(const DisplayGroupInfo &displayGroupInfo)
{
    ...
    MMI_HILOGI("ResetCursorPos cursorPos_.displayId:%{public}d", cursorPos_.displayId);
    return cursorPos_;
}
    ...
    for (auto &currentDisplay : displayGroupInfo.displaysInfo) {
        if ((currentDisplay.screenCombination == OHOS::MMI::ScreenCombination::SCREEN_MAIN)) {
            ...
            MMI_HILOGI("CurDisplayId:%{public}d, oldDisplayId:%{public}d",
                currentDisplay.uniqueId, displayInfo->uniqueId);
            if ((displayInfo->uniqueId != currentDisplay.uniqueId) || (!IsPointerOnCenter(oldPtrPos, currentDisplay))) {
    ...
    for (auto &currentDisplay : displayGroupInfo.displaysInfo) {
        if (cursorDisplayId == currentDisplay.id) {
            ...
            if (isOut && (isChange || pointerActiveRectValid)) {
                ...
                auto displayInfo = GetPhysicalDisplay(cursorPos_.displayId);
                CHKPV(displayInfo);
                IPointerDrawingManager::GetInstance()->SetPointerLocation(static_cast<int32_t>(cursorPos_.cursorPos.x),
                    static_cast<int32_t>(cursorPos_.cursorPos.y), displayInfo->uniqueId);
            }
            if (isChange) {
                CancelMouseEvent();
            }
...
{
    int32_t focWid = GetFocusWindowId();
...
#endif // OHOS_BUILD_ENABLE_ANCO
    MMI_HILOGI("Displays Info size:%{public}lu, focusWindowId:%{public}d",
...
void InputWindowsManager::AdjustDisplayRotation()
{
    ...
    if (cursorPos_.displayDirection != displayInfo->displayDirection) {
        ...
        auto displayInfoTmp = GetPhysicalDisplay(cursorPos_.displayId);
    ...
    bool isDisplayRemoved)
{
    ...
    if (lastDpi_ != DEFAULT_DPI && lastDpi_ != DisplaysInfo[0].dpi) {
...
void InputWindowsManager::DispatchPointer(int32_t pointerAction, int32_t windowId)
{
    ...
    CHKPV(udsServer_);
#if defined(OHOS_BUILD_ENABLE_POINTER) && defined(OHOS_BUILD_ENABLE_POINTER_DRAWING)
    if (!IPointerDrawingManager::GetInstance()->GetMouseDisplayState() && !HasMouseHideFlag()) {
...
std::vector<DisplayInfo> InputWindowsManager::GetDisplayInfoVector() const
{
    std::shared_lock<std::shared_mutex> lock(displayGroupInfoMtx);
    return displayGroupInfo_.displaysInfo;
}

const std::vector<WindowInfo> InputWindowsManager::GetWindowInfoVector() const
{
    std::shared_lock<std::shared_mutex> lock(displayGroupInfoMtx);
...
int32_t InputWindowsManager::UpdateMouseTarget(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    int32_t logicalX = 0;
    int32_t logicalY = 0;
    if (!AddInt32(pointerItem.GetDisplayX(), DisplayInfoX, logicalX)) {
        MMI_HILOGE("The addition of logicalX overflows");
        return RET_ERR;
    }
    if (!AddInt32(pointerItem.GetDisplayY(), DisplayInfoY, logicalY)) {
    ...
    if (IsMouseDrawing(pointerEvent->GetPointerAction())) {
        ...
        IPointerDrawingManager::GetInstance()->DrawPointer(physicalDisplayInfo->uniqueId, physicalX, physicalY,
            dragPointerStyle_, direction);
    }
#endif // OHOS_BUILD_ENABLE_POINTER_DRAWING
    cursorPos_.direction = physicalDisplayInfo->direction;
    cursorPos_.displayDirection = physicalDisplayInfo->displayDirection;
    int64_t endTime = GetSysClockTime();
    if ((endTime - beginTime) > RS_PROCESS_TIMEOUT) {
        MMI_HILOGW("Rs process timeout");
    }
...
int32_t InputWindowsManager::UpdateTouchScreenTarget(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    for (auto &item : windowsInfo) {
        ...
        } else if (extraData_.toolType == SourceTool::FINGER) {
            checkToolType = extraData_.appended && extraData_.sourceType == PointerEvent::SOURCE_TYPE_TOUCHSCREEN &&
                (ConvertToolType(pointerItem.GetToolType()) == extraData_.toolType &&
                extraData_.pointerId == pointerId);
        } else {
            checkToolType = ConvertToolType(pointerItem.GetToolType()) == extraData_.toolType;
        }
    ...
    if (extraData_.toolType == SourceTool::UNKNOWN) {
        ...
        checkExtraData = ConvertToolType(pointerItem.GetToolType()) == extraData_.toolType;
    }
...
Direction InputWindowsManager::GetDisplayDirection(const DisplayInfo *displayInfo)
{
    Direction displayDirection = static_cast<Direction>((
...
{
    auto displaysInfoVector = GetDisplayInfoVector();
    if ((mouseLocation_.displayId < 0) && !displaysInfoVector.empty()) {
...
bool InputWindowsManager::OnDisplayRemovedOrCombiantionChanged(const DisplayGroupInfo &displayGroupInfo)
{
    ...
    MMI_HILOGI("newMainDisplayInfo:%{public}d, oldMainDisplayInfo:%{public}d",
        newMainDisplayInfo.uniqueId, oldMainDisplayInfo.uniqueId);
    if (displayGroupInfo.displaysInfo.size() == displaysInfoVector.size() &&
        newMainDisplayInfo.uniqueId != oldMainDisplayInfo.uniqueId) {
