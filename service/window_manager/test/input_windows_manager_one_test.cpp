...
/* *
 * @tc.name: InputWindowsManagerOneTest_ProcessTouchTracking_003
 * @tc.desc: Test the function ProcessTouchTracking with various pointer events
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(InputWindowsManagerOneTest, InputWindowsManagerOneTest_ProcessTouchTracking_003, TestSize.Level1)
{
    CALL_TEST_DEBUG;
    std::shared_ptr<InputWindowsManager> inputWindowsManager = std::make_shared<InputWindowsManager>();
    std::shared_ptr<PointerEvent> pointerEvent = PointerEvent::Create();
    ...
    // Test when NeedTouchTracking returns false
    pointerEvent->SetPointerAction(PointerEvent::POINTER_ACTION_HOVER_MOVE);
    pointerEvent->bitwise_ = InputEvent::EVENT_FLAG_SHELL;
    EXPECT_NO_FATAL_FAILURE(inputWindowsManager->ProcessTouchTracking(pointerEvent, targetWindow));
 
    // Test when target window ID matches event's target window ID
    pointerEvent->SetPointerAction(PointerEvent::POINTER_ACTION_HOVER_MOVE);
    pointerEvent->SetTargetWindowId(1);
    EXPECT_NO_FATAL_FAILURE(inputWindowsManager->ProcessTouchTracking(pointerEvent, targetWindow));
 
    // Test when pointer item retrieval fails
    pointerEvent->SetPointerId(10);
    EXPECT_NO_FATAL_FAILURE(inputWindowsManager->ProcessTouchTracking(pointerEvent, targetWindow));
 
    // Test when pointer item retrieval succeeds
    pointerEvent->pointers_.clear();
    pointerItem.SetPointerId(1);
    pointerEvent->pointers_.push_back(pointerItem);
    EXPECT_NO_FATAL_FAILURE(inputWindowsManager->ProcessTouchTracking(pointerEvent, targetWindow));
...
/* *
 * @tc.name: InputWindowsManagerOneTest_ProcessTouchTracking_004
 * @tc.desc: Test the function ProcessTouchTracking with different window IDs
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(InputWindowsManagerOneTest, InputWindowsManagerOneTest_ProcessTouchTracking_004, TestSize.Level1)
{
    CALL_TEST_DEBUG;
    std::shared_ptr<InputWindowsManager> inputWindowsManager = std::make_shared<InputWindowsManager>();
    std::shared_ptr<PointerEvent> pointerEvent = PointerEvent::Create();
    ...
    pointerItem.SetDisplayYPos(0.0);
    WindowInfo targetWindow;
    targetWindow.id = -1; // Invalid window ID
 
    // Test with invalid target window ID
    pointerEvent->SetPointerAction(PointerEvent::POINTER_ACTION_HOVER_MOVE);
    pointerEvent->bitwise_ = InputEvent::EVENT_FLAG_SHELL;
    pointerEvent->pointers_.clear();
    pointerItem.SetPointerId(1);
    ...
    // Test with large window ID
    targetWindow.id = INT_MAX;
    EXPECT_NO_FATAL_FAILURE(inputWindowsManager->ProcessTouchTracking(pointerEvent, targetWindow));
}
