...
void AccountManager::OnAddUser(const EventFwk::CommonEventData &data)
{
    ...
    MMI_HILOGI("Add account(%{private}d)", accountId);
...
{
    int32_t accountId = data.GetCode();
    MMI_HILOGI("Remove account(%{private}d)", accountId);
    if (auto iter = accounts_.find(accountId); iter != accounts_.end()) {
        accounts_.erase(iter);
        MMI_HILOGI("Account(%{private}d) has been removed", accountId);
    } else {
        MMI_HILOGW("No account(%{private}d)", accountId);
    }
}
...
{
    int32_t accountId = data.GetCode();
    MMI_HILOGI("Switch to account(%{private}d)", accountId);
    if (currentAccountId_ != accountId) {
        ...
        MMI_HILOGI("Switched to account(%{private}d)", currentAccountId_);
    }
}
} // namespace MMI
} // namespace OHOS
