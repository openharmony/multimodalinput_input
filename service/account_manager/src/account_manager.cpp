...
{
    MMI_HILOGD("[AccountSetting][%{public}d] Setting '%{public}s' has changed", GetAccountId(), key.c_str());
    ReadLongPressTime();
}
...
void AccountManager::OnAddUser(const EventFwk::CommonEventData &data)
{
    ...
    MMI_HILOGD("Add account(%{public}d)", accountId);
...
{
    int32_t accountId = data.GetCode();
    MMI_HILOGD("Remove account(%{public}d)", accountId);
    if (auto iter = accounts_.find(accountId); iter != accounts_.end()) {
        accounts_.erase(iter);
        MMI_HILOGD("Account(%{public}d) has been removed", accountId);
    } else {
        MMI_HILOGW("No account(%{public}d)", accountId);
    }
...
void AccountManager::OnSwitchUser(const EventFwk::CommonEventData &data)
{
    ...
    if (ec == std::errc() && num <= UINT64_MAX) {
        ...
        MMI_HILOGD("Switch to {%{public}" PRIu64 ":%{public}d}", currentDisplayId, accountId);
    } else {
    ...
    if (currentDisplayId == MAIN_DISPLAY_ID && currentAccountId_ != accountId) {
        ...
        MMI_HILOGD("Switched to account(%{public}d)", currentAccountId_);
    }
