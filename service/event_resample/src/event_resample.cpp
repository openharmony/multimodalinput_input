    ...
    int64_t frameTime, bool &deferred, ErrCode &status)
{
    ...
    CheckAndInit(pointerEvent, frameTime, status);
    if (status == ERR_WOULD_BLOCK) {
        MMI_HILOGE("No need to process the pointerEvent");
        return pointerEvent;
    }
    ...
    do {
        ...
        ErrCode res = UpdateEventToPatch(deferred, result, &outEvent);
        if (res == RET_BREAK) {
            MMI_HILOGE("AddEventToPach successful! break the while loop");
            break;
        }
...
void EventResample::CheckAndInit(std::shared_ptr<PointerEvent> pointerEvent, int64_t frameTime, ErrCode &status)
{
    if (frameTime_ <= 0) {
        if (0 != frameTime) {
    ...
    if (nullptr != pointerEvent) {
        ...
        MMI_HILOGD("pointerAction:%{public}d %{public}" PRId64 "%{public}" PRId64, pointerAction,
		   pointerEvent->GetActionTime(), frameTime);
...
ErrCode EventResample::UpdateEventToPatch(bool &deferred, ErrCode& result, MotionEvent** outEvent)
{
...
void EventResample::UpdatePointerEvent(MotionEvent* outEvent)
{
    ...
    for (auto &it : outEvent->pointers) {
        MMI_HILOGD("Output event: %{public}d %{public}d %{public}" PRId64,
		   it.second.coordX, it.second.coordY, outEvent->actionTime);
...
void EventResample::ResampleTouchState(int64_t sampleTime, MotionEvent* event, const MotionEvent* next)
{
    ...
    ReampleTouchCoords(alpha, sampleTime, event, touchState, current, other);
}

void EventResample::ReampleTouchCoords(float alpha, int64_t sampleTime, MotionEvent* event, TouchState& touchState,
