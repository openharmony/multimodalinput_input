...
private:
    ...
    void CheckAndInit(std::shared_ptr<PointerEvent> pointerEvent, int64_t frameTime, ErrCode &status);
    ErrCode UpdateEventToPatch(bool &deferred, ErrCode& result, MotionEvent** outEvent);
    void ReampleTouchCoords(float alpha, int64_t sampleTime, MotionEvent* event, TouchState& touchState,
