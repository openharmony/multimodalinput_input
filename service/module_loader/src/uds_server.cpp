...
{
    static constexpr size_t bufferSize = 512 * 1024;
    static constexpr size_t serverBufferSize = 256 * 1024;
    static constexpr size_t nativeBufferSize = 1024 * 1024;
    if (setsockopt(serverFd, SOL_SOCKET, SO_SNDBUF, &serverBufferSize, sizeof(serverBufferSize)) != 0) {
        MMI_HILOGE("setsockopt serverFd failed, errno:%{public}d", errno);
        return RET_ERR;
    }
    if (setsockopt(serverFd, SOL_SOCKET, SO_RCVBUF, &serverBufferSize, sizeof(serverBufferSize)) != 0) {
        MMI_HILOGE("setsockopt serverFd failed, errno:%{public}d", errno);
        return RET_ERR;
    }
