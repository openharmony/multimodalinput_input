    ...
    HandleEventType eventType, PriorityLevel priorityLevel)
{
    ...
    return sMsgHandler_.OnAddInputHandler(sess, handlerType, eventType, priorityLevel);
}
    ...
    PriorityLevel priorityLevel)
{
    ...
    int32_t ret = delegateTasks_.PostSyncTask(
        std::bind(&MMIService::CheckAddInput, this, pid, handlerType, eventType, priorityLevel));
    if (ret != RET_OK) {
    ...
    PriorityLevel priorityLevel)
{
    auto sess = GetSessionByPid(pid);
    CHKPR(sess, ERROR_NULL_POINTER);
    return sMsgHandler_.OnRemoveInputHandler(sess, handlerType, eventType, priorityLevel);
}
#endif // OHOS_BUILD_ENABLE_INTERCEPTOR || OHOS_BUILD_ENABLE_MONITOR
    ...
    PriorityLevel priorityLevel)
{
    ...
    int32_t pid = GetCallingPid();
    int32_t ret = delegateTasks_.PostSyncTask(
        std::bind(&MMIService::CheckRemoveInput, this, pid, handlerType, eventType, priorityLevel));
    if (ret != RET_OK) {
