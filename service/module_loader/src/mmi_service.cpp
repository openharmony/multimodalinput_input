...
int32_t MMIService::OnGetDevice(int32_t deviceId, std::shared_ptr<InputDevice> inputDevice)
{
    ...
    // Surface the device's bound display (if any) so applications can observe the bindToDisplay
    // result through getDevice/getDeviceInfo/getDeviceInfoSync. -1 means unbound.
    inputDevice->SetDisplayId(WIN_MGR->GetBindDisplayIdByInputDevice(deviceId));
...
ErrCode MMIService::GetCurrentCursorInfo(bool& visible, PointerStyle& pointerStyle)
{
    ...
    if (ret != RET_OK) {
        MMI_HILOGE("PostSyncTask GetCurrentCursorInfo failed, ret:%{public}d", ret);
    }
    return ret;
}
...
ErrCode MMIService::GetUserDefinedCursorPixelMap(std::shared_ptr<PixelMap>& pixelMap)
{
    ...
    if (ret != RET_OK) {
        MMI_HILOGE("PostSyncTask GetUserDefinedCursorPixelMap failed, ret:%{public}d", ret);
    }
    return ret;
}
