...
int32_t MMIService::CheckAddInput(int32_t handlerId, InputHandlerType handlerType)
{
...
{
    CALL_LOG_ENTER;
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::CheckAddInput, this, handlerId, handlerType));
    if (ret != RET_OK) {
...
int32_t MMIService::CheckRemoveInput(int32_t handlerId, InputHandlerType handlerType)
{
    auto sess = GetSessionByPid(GetCallingPid());
    CHKPR(sess, ERROR_NULL_POINTER);
...
{
    CALL_LOG_ENTER;
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::CheckRemoveInput, this, handlerId, handlerType));
    if (ret != RET_OK) {
...
int32_t MMIService::CheckMarkConsumed(int32_t monitorId, int32_t eventId)
{
    auto sess = GetSessionByPid(GetCallingPid());
    CHKPR(sess, ERROR_NULL_POINTER);
...
{
    CALL_LOG_ENTER;
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::CheckMarkConsumed, this, monitorId, eventId));
    if (ret != RET_OK) {
