...
void KeyGestureManager::Handler::Trigger(std::shared_ptr<KeyEvent> keyEvent)
{
    ...
    if (timerId_ < 0) {
        MMI_HILOGE("[Handler] AddTimer fail");
    }
}
...
bool KeyGestureManager::LongPressCombinationKey::Intercept(std::shared_ptr<KeyEvent> keyEvent)
{
    if ((keys_.find(keyEvent->GetKeyCode()) != keys_.end()) &&
        ...
        if (IsActive()) {
            std::ostringstream output;
            Dump(output);
            MMI_HILOGI("[LongPressCombinationKey] %{private}s is active now", output.str().c_str());
            return true;
        }
        if (!IsWorking()) {
            std::ostringstream output;
            Dump(output);
            MMI_HILOGI("[LongPressCombinationKey] Switch off %{private}s", output.str().c_str());
            return false;
        }
        if (handlers_.empty()) {
            std::ostringstream output;
            Dump(output);
            MMI_HILOGI("[LongPressCombinationKey] No handler for %{private}s", output.str().c_str());
            return false;
        }
...
void KeyGestureManager::LongPressCombinationKey::TriggerAll(std::shared_ptr<KeyEvent> keyEvent)
{
    ...
    std::ostringstream output;
    Dump(output);
    MMI_HILOGI("[LongPressCombinationKey] trigger %{private}s", output.str().c_str());
    ...
    std::function<void(std::shared_ptr<KeyEvent>)> callback)
{
    ...
    MMI_HILOGD("No key gesture matched, pid:%{public}d, finalKey:%{private}d",
...
bool KeyGestureManager::Intercept(std::shared_ptr<KeyEvent> keyEvent)
{
    ...
    for (auto iter = keyGestures_.begin(); iter != keyGestures_.end(); ++iter) {
        ...
        if ((*iter)->Intercept(keyEvent)) {
            std::ostringstream output;
            (*iter)->Dump(output);
            MMI_HILOGI("Intercepted by %{private}s", output.str().c_str());
