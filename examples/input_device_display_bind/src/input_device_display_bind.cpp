...
constexpr int32_t ARRRY_LENGTH = 5;
enum Option {
    QUERY = 'q',
    SET = 's',
    HELP = 'h'
...
void QueryDisplayBindInfo()
{
    ...
    std::vector<std::array<std::string, ARRRY_LENGTH>> arrStrings;
    std::array<std::string, ARRRY_LENGTH> arr0 = { "No.", "Input device Id", "Input device Name", "Display id",
        "Display name" };
    arrStrings.push_back(arr0);
    for (size_t i = 0; i < infos.size(); ++i) {
        const auto &info = infos[i];
        std::array<std::string, ARRRY_LENGTH> arr;
    ...
    std::array<size_t, ARRRY_LENGTH> arrWidth{};
