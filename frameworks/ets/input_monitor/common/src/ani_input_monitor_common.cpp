...
int32_t TaiheMonitorConverter::ThreeFingersTapToTaihe(const PointerEvent &pointerEvent, TaiheThreeFingersTap &out)
{
    ...
    if (ret != RET_OK) {
        MMI_HILOGE("MultiTapActionToTaihe error");
        return ret;
    }
    out.type = type;
    return ret;
...
int32_t TaiheMonitorConverter::FingerprintEventToTaihe(const PointerEvent &pointerEvent, TaiheFingerprintEvent &out)
{
    ...
    if (ret != RET_OK) {
        MMI_HILOGE("FingerprintActionToTaihe error");
        return ret;
    }
    out.action = type;
...
bool TaiheMonitorConverter::ParseRects(ani_object aniRects, std::vector<Rect> &rects, int32_t maxNum)
{
    ...
    if (maxNum <= 0) {
        MMI_HILOGE("maxNum invaild, maxNum: %{public}d", maxNum);
        return false;
    }
    ...
    for (ani_size i = 0; i < size; i++) {
        ...
        if (status != ANI_OK) {
            MMI_HILOGE("Get rect ref failed, i:%{public}zu ret: %{public}d", i, status);
            return false;
        }
        Rect per;
        if (!ParseRect(env, static_cast<ani_object>(rectRef), per)) {
            MMI_HILOGE("Parse rect failed, i:%{public}zu", i);
            return false;
        }
        rects.push_back(per);
    }
