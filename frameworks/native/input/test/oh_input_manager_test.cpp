...
static void MyKeyEventCallback(const Input_KeyEvent* keyEvent)
{
    if (keyEvent != nullptr) {
        g_interceptorTriggered = true;
        MMI_HILOGI("[TEST] MyKeyEventCallback triggered, keyCode=%{public}d", keyEvent->keyCode);
    }
}
static void DummyCallback(const Input_KeyEvent* keyEvent)
{
    MMI_HILOGI("[TEST] DummyCallback invoked.");
}
namespace OHOS {
namespace MMI {
namespace {
