...
napi_value JsInputDeviceContext::GetKeystrokeAbility(napi_env env, napi_callback_info info)
{
    ...
    if (size < 1 || size > 5) {
        MMI_HILOGE("the number of parameters is incorrect, maximum 5");
        napi_throw_error(env, nullptr, "JsInputDeviceContext: the number of parameters is incorrect, maximum 5");
        return nullptr;
    }
