...
namespace OHOS {
namespace MMI {
class JsInputMonitorHistogram final {
private:
    struct HistogramActions {
        std::function<void(bool)> on_ {};
        std::function<void(bool)> off_ {};
        std::function<void(int32_t)> onError_ {};
        std::function<void(int32_t)> offError_ {};
    };

    using JsMonitorHistogramActionMap = std::unordered_map<std::string, HistogramActions>;

public:
    static void HistogramOn(const std::string &type, bool sample);
    static void HistogramOff(const std::string &type, bool sample);
    static void HistogramOnError(const std::string &type, int32_t errorCode);
    static void HistogramOffError(const std::string &type, int32_t errorCode);
...
private:
    ...
    static const JsMonitorHistogramActionMap histogramActions_;
};
} // namespace MMI
} // namespace OHOS
