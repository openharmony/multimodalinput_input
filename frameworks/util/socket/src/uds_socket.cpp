...
void UDSSocket::OnReadPackets(CircleStreamBuffer& circBuf, PacketCallBackFun callbackFun)
{
    ...
    for (int32_t i = 0; i < ONCE_PROCESS_NETPACKET_LIMIT; i++) {
        ...
        if (head->size < 0 || head->size > MAX_PACKET_BUF_SIZE) {
            MMI_LOGF("Packet header parsing error, and this error cannot be recovered. The buffer will be reset."
        ...
        if ((head->size > 0) && (!pkt.Write(&buf[headSize], head->size))) {
            MMI_LOGW("Error writing data in the NetPacket. It will be retried next time. messageid: %{public}d,"
                "size:%{public}d", head->idMsg, head->size);
            break;
        }
        if (!circBuf.SeekReadPos(pkt.GetPacketLength())) {
            MMI_LOGW("Set read position error, and this error cannot be recovered, and the buffer will be reset."
