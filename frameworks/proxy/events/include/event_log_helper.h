        ...
        for (const auto &pointerId : pointerIds) {
            ...
            if (!IsBetaVersion()) {
                MMI_HILOG_HEADER(LOG_INFO, lh, "PI:%{public}d, IP:%{public}d, P:%{public}.2f, MF:%{public}d, "
                    "WI:%{public}d, OPI:%{public}d, SI:%{public}s",
                ...
                if (event->HasFlag(InputEvent::EVENT_FLAG_PRIVACY_MODE)) {
                    MMI_HILOG_HEADER(LOG_INFO, lh, "PI:%{public}d, DT:%{public}" PRId64 ", IP:%{public}d,DX:%d, DY:%d,"
                        ...
                        "DXP:%f, DYP:%f, WXP:%f, WYP:%f, OPI:%{public}d",
        ...
        for (const auto &pointerId : pointerIds) {
            ...
            if (!IsBetaVersion()) {
                ...
                if (event->HasFlag(InputEvent::EVENT_FLAG_PRIVACY_MODE)) {
                    MMI_HILOG_HEADER(LOG_DEBUG, lh,"PI:%{public}d, DT:%{public}" PRId64 ", IP:%{public}d, DX:%d, DY:%d"
                        ...
                        "ToolType:%{public}d, LA:%{public}d, SA:%{public}d, RawDx:%d, RawDy:%d",
                        pointerId, item.GetDownTime(), item.IsPressed(), item.GetDisplayX(), item.GetDisplayY(),
            ...
            if (!IsBetaVersion()) {
                ...
                MMI_HILOG_HEADER(LOG_DEBUG, lh,
                    ...
                    "P:%{public}.2f, MF:%{public}d, ToolType:%{public}d, LA:%{public}d, SA:%{public}d, "
                    ...
                    item.GetPressure(), item.GetMoveFlag(), item.GetToolType(), item.GetLongAxis(), item.GetShortAxis(),
                    item.GetRawDx(), item.GetRawDy());
            }
        }
        std::vector<int32_t> pressedKeys = event->GetPressedKeys();
