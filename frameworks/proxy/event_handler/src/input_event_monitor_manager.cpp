    ...
    std::function<void (std::shared_ptr<KeyEvent>)> keyEventMonitor)
{
    ...
    if (ret != RET_OK) {
        MMI_HILOGE("MultimodalEventHandler send msg failed");
    ...
    MMI_HILOGD("MonitorId:%{public}d", monitorId);
    return item.id;
}
...
void InputEventMonitorManager::RemoveInputEventMontior(int32_t monitorId)
{
    ...
    if (it != monitors_.end()) {
        ...
        MMI_HILOGD("MonitorId:%{public}d removed", monitorId);
    }
}
...
int32_t InputEventMonitorManager::OnTouchpadMonitorInputEvent(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    MMI_HILOGD("Monitor-clienteventTouchpad:time:%{public}" PRId64 ","
