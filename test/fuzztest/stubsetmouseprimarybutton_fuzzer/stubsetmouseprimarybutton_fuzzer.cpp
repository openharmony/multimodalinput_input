...
#include "stubsetmouseprimarybutton_fuzzer.h"
...
#undef MMI_LOG_TAG
#define MMI_LOG_TAG "StubSetMousePrimaryButtonFuzzTest"
...
bool StubSetMousePrimaryButtonFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<MMIService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(MMI::MultimodalinputConnectInterfaceCode::SET_MOUSE_PRIMARY_BUTTON), datas, reply, option);
    return true;
}
} // namespace OHOS
