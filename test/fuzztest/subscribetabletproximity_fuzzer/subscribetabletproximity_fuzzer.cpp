...
#include "subscribetabletproximity_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "SubscribeTabletProximityFuzzTest"
...
bool SubscribeTabletProximityFuzzTest(FuzzedDataProvider &provider)
{
    ...
    (void)MMIService::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(
            IMultimodalInputConnectIpcCode::COMMAND_SUBSCRIBE_TABLET_PROXIMITY),
        datas, reply, option);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    (void)OHOS::MMI::SubscribeTabletProximityFuzzTest(provider);
    return 0;
}
} // namespace MMI
} // namespace OHOS