...
#include "setmouseacceleratemotionswitch_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "SetMouseAccelerateMotionSwitchFuzzTest"
...
bool SetMouseAccelerateMotionSwitchFuzzTest(FuzzedDataProvider &provider)
{
    MessageParcel datas;
    ...
    (void)MMIService::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(
            IMultimodalInputConnectIpcCode::COMMAND_SET_MOUSE_ACCELERATE_MOTION_SWITCH),
        datas, reply, option);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    (void)OHOS::MMI::SetMouseAccelerateMotionSwitchFuzzTest(provider);
    return 0;
}
