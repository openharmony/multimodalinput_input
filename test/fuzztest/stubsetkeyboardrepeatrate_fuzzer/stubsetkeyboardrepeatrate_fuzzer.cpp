...
#include "stubsetkeyboardrepeatrate_fuzzer.h"
...
#undef MMI_LOG_TAG
#define MMI_LOG_TAG "StubSetKeyboardRepeatRateFuzzTest"
...
bool StubSetKeyboardRepeatRateFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<MMIService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(MMI::MultimodalinputConnectInterfaceCode::SET_KEYBOARD_REPEAT_RATE), datas, reply, option);
    return true;
}
} // namespace OHOS
