...
#undef MMI_LOG_TAG
#define MMI_LOG_TAG "InputEvent2FuzzTest"

namespace OHOS {
namespace MMI {
void InputEvent2FuzzTest(const uint8_t *data, size_t size)
{
    ...
    inputEvent.SetAction(action);

    int64_t time = provider.ConsumeIntegral<int64_t>();
    inputEvent.SetActionStartTime(time);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::MMI::InputEvent2FuzzTest(data, size);
    return 0;
}