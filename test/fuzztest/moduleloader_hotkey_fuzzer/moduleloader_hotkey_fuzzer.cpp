...
#include "moduleloader_hotkey_fuzzer.h"
...
{
    std::vector<KeyOption> opts;
    MMIService::GetInstance()->GetAllSystemHotkeys(opts);
    ...
    int32_t cnt = fdp.ConsumeIntegral<int32_t>();
    std::vector<std::shared_ptr<PointerEvent>> list;
    MMIService::GetInstance()->QueryPointerRecord(cnt, list);
}
