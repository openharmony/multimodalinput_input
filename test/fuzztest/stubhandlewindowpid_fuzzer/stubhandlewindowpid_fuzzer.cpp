...
#include "stubhandlewindowpid_fuzzer.h"
...
void StubHandleWindowPidFuzzTest(const uint8_t *data, size_t size)
{
    FuzzedDataProvider provider(data, size);

    int32_t windowId = provider.ConsumeIntegral<int32_t>();
    int32_t ywindowPid = provider.ConsumeIntegral<int32_t>();
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::MMI::StubHandleWindowPidFuzzTest(data, size);
    return 0;
}
