...
#include "removeinputeventobserver_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "RemoveInputEventObserverFuzzTest"
...
bool RemoveInputEventObserverFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<MMIService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(MMI::MultimodalinputConnectInterfaceCode::RMV_INPUT_EVENT_OBSERVER), datas, reply, option);
    return true;
}
} // namespace OHOS
