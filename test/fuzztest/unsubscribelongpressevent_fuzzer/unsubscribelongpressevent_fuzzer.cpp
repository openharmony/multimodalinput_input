...
#include "unsubscribelongpressevent_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "UnsubscribeLongPressEventFuzzTest"
...
bool UnsubscribeLongPressEventFuzzTest(FuzzedDataProvider &provider)
{
    ...
    (void)MMIService::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(
            IMultimodalInputConnectIpcCode::COMMAND_UNSUBSCRIBE_LONG_PRESS_EVENT),
        datas, reply, option);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    (void)OHOS::MMI::UnsubscribeLongPressEventFuzzTest(provider);
    return 0;
}
} // namespace MMI
} // namespace OHOS