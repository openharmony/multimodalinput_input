...
#include "getkeyboardrepeatrate_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "GetKeyBoardRepeatRateFuzzTest"
...
bool GetKeyBoardRepeatRateFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<MMIService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(MMI::MultimodalinputConnectInterfaceCode::GET_KEYBOARD_REPEAT_RATE), datas, reply, option);
    return true;
}
} // namespace OHOS
