...
{
    return (parcel.WriteString(remoteNetworkId) && parcel.WriteInt32(startDeviceId) && parcel.WriteInt32(userData) &&
        parcel.WriteBool(checkPermission));
}

bool StartCooperateParam::Unmarshalling(MessageParcel &parcel)
{
    return (parcel.ReadString(remoteNetworkId) && parcel.ReadInt32(startDeviceId) && parcel.ReadInt32(userData) &&
...
{
    return (parcel.WriteBool(isUnchained) && parcel.WriteBool(checkPermission) && parcel.WriteInt32(userData));
}

bool StopCooperateParam::Unmarshalling(MessageParcel &parcel)
{
    return (parcel.ReadBool(isUnchained) && parcel.ReadBool(checkPermission) && parcel.ReadInt32(userData));
}
...
{
    return (parcel.WriteInt32(userData) && parcel.WriteString(networkId) && parcel.WriteBool(checkPermission));
}

bool GetCooperateStateParam::Unmarshalling(MessageParcel &parcel)
{
    return (parcel.ReadInt32(userData) && parcel.ReadString(networkId) && parcel.ReadBool(checkPermission));
}
...
RegisterHotAreaListenerParam::RegisterHotAreaListenerParam(int32_t userData, bool checkPermission)
    : userData(userData), checkPermission(checkPermission)
{
}

bool RegisterHotAreaListenerParam::Marshalling(MessageParcel &parcel) const
{
    return parcel.WriteInt32(userData) && parcel.WriteBool(checkPermission);
}

bool RegisterHotAreaListenerParam::Unmarshalling(MessageParcel &parcel)
{
    return parcel.ReadInt32(userData) && parcel.ReadBool(checkPermission);
}
